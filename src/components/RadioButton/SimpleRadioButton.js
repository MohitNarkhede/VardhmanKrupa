...
export default class RadioForm extends React.Component {
  ...
  updateIsActiveIndex(index) {
    ...
    this.props.onPress(this.props.data[index], index)
  }
  ...
  _renderButton(obj, i) {
    return (
      <RadioButton
        ...
        labelStyle={{fontFamily: this.state.is_active_index === i ? Fonts.Bold : Fonts.Regular, fontSize: 16, marginRight: 20}}
  ...
  render() {
    ...
    if (this.props.data.length) {
      render_content = this.props.data.map(this._renderButton)
    } else {
...
export class RadioButtonInput extends React.Component {
  ...
  render () {
    var innerSize = {width: 12, height:12, borderRadius: 12/2 }
    var outerSize = {width: 12+10, height:12+10, borderRadius: (12+10)/2 }
